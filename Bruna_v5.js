function readInput(){   //start function
    ...
    for (var dirt_patch of dirty_patches) {
        ...
        dirty.array[x][y]= true; //takes the array in part 3 below and changes boolean "falses" to "true" if the coordinates match the array
...
// check for dirt when moving
    if(dirty_array[x][y] === true) {
        console.log('Dirt found at: ', p.x, p.y);
        hooveredDirt += 1;
        }
...
for (var i=0; i < room_size_x; i++) {  //for loop that says for all x less than x-axis...
    ...
    dirty.push(row); //array exact size of room
}
